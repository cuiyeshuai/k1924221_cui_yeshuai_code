...
def json_to_encoding_extend(x: str, iri, forward, qualified_names):
    output = ({}, {})  # (contents, relations)
    file = json.decoder.JSONDecoder().decode(x)
    if "prefix" in file:
        if iri:
                ...
                if rec_type in PROV_NODE: 
                    if "prov:type" in content:
                        ...
                        specific_labels = frozenset([assert_type.get("$") for assert_type in content["prov:type"] 
                            if type(assert_type) is dict and assert_type.get("type") in qualified_names])
                        if iri:
                            specific_labels = specific_labels.union(frozenset([prefix[label.split(":", 1)[0]] + label.split(":", 1)[1]
                                for label in specific_labels if label.split(":", 1)[0] in prefix]))
                    if "prov:role" in content:
                        if isinstance(content.get("prov:role"), dict):
                            content["prov:role"] = [content["prov:role"]] # make it a list
                        specific_labels = frozenset([assert_type.get("$") for assert_type in content["prov:role"] 
                            if type(assert_type) is dict and assert_type.get("type") in qualified_names])
                        if iri:
                            specific_labels = specific_labels.union(frozenset([prefix[label.split(":", 1)[0]] + label.split(":", 1)[1]
                                for label in specific_labels if label.split(":", 1)[0] in prefix]))
                    ...
                    if relation is None:
                        continue
                    edge = content.get("prov:" + relation[0]), content.get("prov:" + relation[1])
                    specific_labels = frozenset()
                    if "prov:type" in content:
                        if isinstance(content.get("prov:type"), dict):
                            content["prov:type"] = [content["prov:type"]]
                        specific_labels = specific_labels.union(frozenset([assert_type.get("$") for assert_type in content["prov:type"]
                            if type(assert_type) is dict and assert_type.get("type") in qualified_names]))
                    if "prov:role" in content:
                        if isinstance(content.get("prov:role"), dict):
                            content["prov:role"] = [content["prov:role"]]
                        specific_labels = specific_labels.union(frozenset([assert_type.get("$") for assert_type in content["prov:role"]
                            if type(assert_type) is dict and assert_type.get("type") in qualified_names]))
                    if None in edge:
                        if rec_type == PROV_DERIVATION:
                            if "prov:Revision" or "prov:Quotation" or "prov:PrimarySource" in specific_labels:
...
def type_generate_mixed(x, level, specific_labels_node, specific_labels_edge):
    ...
    for i in range(1, level+1):
        for source in x[1]:
            for destination, edge_type in x[1][source]: #iterate through all edges
                if destination in prov_types[i-1]: #if the destination is in the previous level
                    if prov_types[i].get(source) is None:
